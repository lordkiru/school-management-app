...
function ParentPay() {
  ...
  const handleLookup = async (e) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/fees/public/lookup/${admissionNumber}`);
  ...
  const handlePay = async (feeId) => {
    setPayingId(feeId);
    try {
      const res = await fetch(`http://localhost:5000/fees/public/${feeId}/initiate-payment`, {
        method: 'POST',
      });
      const data = await res.json();
